...
def apply_strategy(strategy_func, stock_df):
    ...
    try:
        ...
        if isinstance(result, pd.DataFrame):
            return result.reindex(stock_df.index)  # Ensure same index
...
def generate_all_signals(stock_df, strategies):
    """Applies all loaded strategy functions using ThreadPoolExecutor, ensuring consistency."""
    ...
    all_signals_df = pd.concat([stock_df] + valid_results, axis=1)
    return all_signals_df
#%%

strategies_file = r"C:\Users\zebfr\Documents\All_Files\TRADING\Trading_Bot\features\TOS_Strategies\all_strategies.py"    # Output file with combined scripts
