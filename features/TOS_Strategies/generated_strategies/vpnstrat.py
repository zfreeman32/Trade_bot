...
def vpn_signals(df, length=14, ema_length=14, average_length=10, rsi_length=14, volume_average_length=50, highest_length=14, atr_length=14, factor=1, critical_value=10, rsi_max_overbought_level=90, num_atrs=1, average_type='simple'):
    vwap = VolumeWeightedAveragePrice(high=df['High'], low=df['Low'], close=df['Close'], volume=df['Volume'], window=average_length)
    ...
    buy_condition = (
        ...
        (RSIIndicator(close=df['Close'], window=rsi_length).rsi() < rsi_max_overbought_level)
    )
    ...
    sell_condition = (
        ...
        (df['Close'] < df['Close'].rolling(window=highest_length).max() - num_atrs * 
         AverageTrueRange(high=df['High'], low=df['Low'], close=df['Close'], window=atr_length).average_true_range())
    )
